...
CUDA_CALLABLE inline uint64_t mesh_query_bvh_make_node(const BVHPackedNodeHalf& lower, const BVHPackedNodeHalf& upper)
{
    return (uint64_t(lower.b) << 62) | (uint64_t(upper.i) << 31) | uint64_t(lower.i);
}

CUDA_CALLABLE inline uint64_t mesh_query_bvh_load_node(const BVH& bvh, int node_index)
{
    ...
    return mesh_query_bvh_make_node(lower, upper);
}

CUDA_CALLABLE inline bool mesh_query_bvh_is_leaf(uint64_t node) { return (node >> 62) != 0; }

CUDA_CALLABLE inline int mesh_query_bvh_lower_payload(uint64_t node) { return int(node & 0x7fffffffu); }

CUDA_CALLABLE inline int mesh_query_bvh_upper_payload(uint64_t node) { return int((node >> 31) & 0x7fffffffu); }
...
)
{
    ...
    int stack_size = 0;
    uint64_t cur_node = mesh_query_bvh_load_node(mesh.bvh, (root == -1) ? *mesh.bvh.root : root);
    ...
    while (true) {
        if (mesh_query_bvh_is_leaf(cur_node)) {
            const int primitive_begin = mesh_query_bvh_lower_payload(cur_node);
            const int primitive_end = mesh_query_bvh_upper_payload(cur_node);
        ...
        const int left_index = mesh_query_bvh_lower_payload(cur_node);
        const int right_index = mesh_query_bvh_upper_payload(cur_node);
        ...
        if (h0 && h1) {
            ...
            if (stack_size >= BVH_QUERY_STACK_SIZE)
                break;
            const uint64_t left_node = mesh_query_bvh_make_node(left_lower, left_upper);
            const uint64_t right_node = mesh_query_bvh_make_node(right_lower, right_upper);
            stack[stack_size++] = near_left ? right_node : left_node;
            cur_node = near_left ? left_node : right_node;
        } else if (h0) {
            cur_node = mesh_query_bvh_make_node(left_lower, left_upper);
        } else if (h1) {
            cur_node = mesh_query_bvh_make_node(right_lower, right_upper);
        } else {
